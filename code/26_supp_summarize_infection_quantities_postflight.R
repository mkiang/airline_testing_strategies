## 26_supp_summarize_infection_quantities_postflight.R ----
##
...
null_results <-
    readRDS(here::here("data_raw", "raw_simulations_no_testing_postflight.RDS"))
...
temp_holder <- foreach::foreach(s = testing_scenarios) %dopar% {
    temp_x <-
        readRDS(here::here("data_raw", sprintf("raw_simulations_%s_postflight.RDS", s)))
    ...
    saveRDS(dplyr::bind_rows(temp_x_list),
            here::here(
                "data_raw",
                sprintf("processed_simulations_wide_%s_postflight.RDS", s)
            ),
            compress = "xz")
}
...
results_quarantine <-
    foreach::foreach(i = 1:NROW(quarantine_comparisons)) %dopar% {
        ...
        base_x <-
            readRDS(here::here(
                "data_raw",
                sprintf("processed_simulations_wide_%s_postflight.RDS", b)
            ))
        comp_x <-
            readRDS(here::here(
                "data_raw",
                sprintf("processed_simulations_wide_%s_postflight.RDS", comp)
            ))
...
saveRDS(summarized_results,
        here::here("data", "summarized_results_postflight.RDS"),
        compress = "xz")
